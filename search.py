...
class Hypothesis(object):
    def __init__(self, state_dict, seq, score, END):
...
class Beam(object):
    ...
    def __init__(self, beam_size, min_time_step, max_time_step, hypotheses, END):
    ...
    def update(self, new_states, last_steps):
        ...
        for idx, (prev_hyp_idx, token, score) in enumerate(candidates):
            ...
            new_hyps.append(Hypothesis(state, seq, score, self.END))
        ...
        for idx, beam in enumerate(beams):
            if not beam.completed():
                for hyp in beam.hypotheses:
                    ...
                    indices.append(idx)
                    concept_batch_index.append(idx)
                    batch_index_count += 1
