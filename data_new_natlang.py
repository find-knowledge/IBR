...
class RRProcessor(DataProcessor):
    ...
    def get_test_examples(self, data_dir):
        ...
        return self._create_examples(self._read_jsonl(os.path.join(data_dir, "test.jsonl")),
                                     ...
                                     natlang_mappings, "test")

    def get_labels(self):
        return [True, False]

    # Unconstrained training, use this for ablation
    def _get_node_edge_label_no_natlang(self, id, proofs, sentence_scramble, nfact, nrule):
    ...
    def _create_examples(self, records, meta_records, natlang_mappings, data_type):
        ...
        for (i, record) in enumerate(records):
            ...
            context = context.replace('be ', '')
            context = context.replace('a ', '')
            context = context.replace('also ', '')
            ...
            for (j, question) in enumerate(record["questions"]):
                ...
                # if isinstance(QLen, int) and data_type == 'train':
                #     if QLen > 24:
                #         count += 1
                #         continue
                ...
                if "NatLang" not in id:
                    ...
                    if data_type != 'train':
                        continue
                    component_index_map, nodes, edges, rule_facts_list, str_dic = self._get_node_edge_label_no_natlang(id, proofs, sentence_scramble, nfact, nrule)
                    sent_offset = [k for k in range(len(rule_facts_list) - 1)]
                else:
                    component_index_map, nodes, edges, rule_facts_list, str_dic = self._get_node_edge_label_natlang(id, proofs, natlang_mappings)
                    natlang_mapping = natlang_mappings[id.split("-")[1]]
                    sent_offset = []
                    sid_sents_map = {}
                    for rf_id, (sid, orig_sents) in natlang_mapping.items():
                    ...
                    for offset in sent_offset:
                        sentence_tokens = ' '.join(sentences[curr_offset:(offset + 1)])
                        if sentence_tokens == '':
                            is_error = True
                            error_tokenization += 1
                            # print('error_sentence_tokens: ' + ' '.join(sentences[curr_offset:(offset + 1)]))
                            # print(' '.join(sentences[curr_offset:(offset + 1)]) == '')
                            break
...
def batchify(data, args, tokenizer, sep_token_extra=True):
    ...
    for example_index, example in enumerate(examples):
        ...
        sentence_list = []
        for offset in sent_offset:
            sentence_list.append(' '.join(sentences[curr_offset:(offset + 1)]))
            sentence_tokens = tokenizer.tokenize(' '.join(sentences[curr_offset:(offset + 1)]))
...
class DataLoader(object):
    ...
    def __iter__(self):
        ...
        print('all error_tokenization: ' + str(error_tokenization))
        # r = random.random
